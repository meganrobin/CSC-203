...
public interface PathingStrategy {
    ...
    /**
     * A static Constant: it's a lambda function that returns the cardinal and intercardinal neighbors of a given point as a stream.
     * Example Usage:
     * Stream<Point> neighbors = PathingStrategy.CARD_AND_INTERCARD_NEIGHBORS.apply(new Point(0, 0));
     */
    static final Function<Point, Stream<Point>> CARD_AND_INTERCARD_NEIGHBORS =
        point ->
                Stream.<Point>builder()
                        .add(new Point(point.x, point.y - 1))
                        .add(new Point(point.x, point.y + 1))
